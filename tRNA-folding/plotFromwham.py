...
fstring=str(str(getcwd())+'/')

regnames=['ach','asl','dloop','tloop','varloop','pseuknot']
...
for yl in coldict.keys():
    ...
    for xl in coldict.keys():
        ...
        xaxis=str(xl)
        if (xaxis != yaxis ):
            fname=str(fstring+'FE'+ xaxis + yaxis)
        ...
        #print(str(np.shape(mat)))
            ...
            #xedges=np.arange(df[0].min(), df[0].max(), 0.1 )
            #yedges=np.arange(df[1].min(), df[1].max(), 0.1 )


        
            axes[coldict[yaxis],coldict[xaxis]].set_xlabel(xaxis)
            axes[coldict[yaxis],coldict[xaxis]].set_ylabel(yaxis)
            #axes[coldict[yaxis],coldict[xaxis]].set_xticks(table.columns)
            #axes[coldict[yaxis],coldict[xaxis]].set_yticks(table.index)
            axes[coldict[yaxis],coldict[xaxis]].invert_yaxis()

            im=axes[coldict[yaxis],coldict[xaxis]].imshow(table,vmin=-8.0,vmax=8.0,aspect='auto', 
            extent= [df[0].min(), df[0].max(),df[1].min(),df[1].max() ] , interpolation='gaussian' )
            axes[coldict[yaxis],coldict[xaxis]].invert_yaxis()
            ...
            axes[coldict[yaxis],coldict[xaxis]].axis('off')
    #axes[coldict[yaxis],coldict[xaxis]].set_xlabel(xaxis)
    #axes[coldict[yaxis],coldict[xaxis]].set_ylabel(yaxis)
    #print(xaxis +' '+ yaxis+' '+str(max(xarray)) +' '+str(max(yarray)))
